# Streamlit UI per LLM con Export Conversazioni Multi-Formato
# Versione 1.2.0 - Export Conversazioni
# Gilles DeepAiUG - Gennaio 2025
# 
# Novità v1.2.0:
# - 📥 Export Markdown (.md) - per blog, Obsidian, Notion
# - 📥 Export JSON (.json) - per elaborazione programmata
# - 📥 Export TXT (.txt) - backup semplice
# - 📥 Export PDF (.pdf) - documenti stampabili
# - 👁️ Preview export prima del download
# - 📝 Nome file personalizzabile
# - 📊 Selezione contenuto (completo o range)
...
# ============================================================================
# FUNZIONI API KEYS (invariate da v1.1.1)
# ============================================================================
...
# ============================================================================
# FUNZIONI PERSISTENZA (invariate da v1.1.1)
# ============================================================================
...
# ============================================================================
# FUNZIONI EXPORT - NUOVE v1.2.0
# ============================================================================

def get_messages_for_export(content_option: str) -> List[Dict[str, Any]]:
    """Ritorna messaggi filtrati secondo opzione selezionata."""
    messages = st.session_state.get("messages", [])
...
def export_to_markdown(messages: List[Dict[str, Any]], metadata: Dict[str, Any]) -> str:
    ...
    # Header
    lines.append("# Conversazione LLM")
    lines.append("")
    lines.append(f"**Data creazione:** {metadata.get('created_at', 'N/A')}")
    lines.append(f"**Ultimo aggiornamento:** {metadata.get('last_updated', 'N/A')}")
    lines.append(f"**Modello:** {metadata.get('model', 'N/A')}")
    lines.append(f"**Provider:** {metadata.get('provider', 'N/A')}")
    lines.append(f"**Messaggi:** {len(messages)} | **Token stimati:** ~{metadata.get('tokens', 0)}")
    lines.append("")
    lines.append("---")
    lines.append("")
    ...
    for msg in messages:
        ...
        if role == "user":
            lines.append(f"## 👤 Tu - {time_str}")
        else:
            model_label = f" ({model_used})" if model_used else ""
            lines.append(f"## 🤖 AI{model_label} - {time_str}")
    ...
    # Footer
    lines.append(f"*Generato con Datapizza Streamlit Interface v1.2.0 il {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}*")
...
def export_to_json(messages: List[Dict[str, Any]], metadata: Dict[str, Any]) -> str:
    ...
    export_data = {
        "export_info": {
            ...
            "exporter": "Datapizza Streamlit Interface v1.2.0",
            "format_version": "1.0"
        },
        "conversation": {
            "id": metadata.get("conversation_id", "unknown"),
            ...
            "stats": {
                ...
                "tokens_estimate": metadata.get("tokens", 0)
            }
        }
    }
...
def export_to_txt(messages: List[Dict[str, Any]], metadata: Dict[str, Any]) -> str:
    ...
    for msg in messages:
        ...
        if role == "user":
            lines.append(f"[{time_str}] Tu:")
        else:
            model_label = f" ({model_used})" if model_used else ""
            lines.append(f"[{time_str}] AI{model_label}:")
    ...
    # Footer
    lines.append("=" * 80)
    lines.append(f"Generato con Datapizza Streamlit Interface v1.2.0")
    lines.append(f"Export: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    lines.append("=" * 80)
...
def export_to_pdf(messages: List[Dict[str, Any]], metadata: Dict[str, Any]) -> bytes:
    ...
    try:
        ...
        # Title
        story.append(Paragraph("Conversazione LLM", title_style))
        story.append(Spacer(1, 0.2 * inch))
        
        # Metadata
        story.append(Paragraph(f"<b>Data creazione:</b> {metadata.get('created_at', 'N/A')}", normal_style))
        story.append(Paragraph(f"<b>Modello:</b> {metadata.get('model', 'N/A')}", normal_style))
        story.append(Paragraph(f"<b>Provider:</b> {metadata.get('provider', 'N/A')}", normal_style))
        story.append(Paragraph(f"<b>Messaggi:</b> {len(messages)}", normal_style))
        ...
        for msg in messages:
            ...
            if role == "user":
                header = f"👤 Tu - {time_str}"
            else:
                header = f"🤖 AI - {time_str}"
            
            story.append(Paragraph(f"<b>{header}</b>", heading_style))
            story.append(Paragraph(content, normal_style))
            story.append(Spacer(1, 0.2 * inch))
        
        # Footer
        story.append(Spacer(1, 0.3 * inch))
        footer_text = f"<i>Generato con Datapizza Streamlit Interface v1.2.0 - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</i>"
        ...
        st.error("📕 PDF export richiede 'reportlab'. Installa con: pip install reportlab")
        return None
    except Exception as e:
...
# ============================================================================
# FUNZIONI CONVERSAZIONE (invariate da v1.1.1)
# ============================================================================
...
st.set_page_config(
    page_title="Datapizza → LLM Chat v1.2.0", 
    layout="wide",
...
# ============================================================================
# SIDEBAR - Configurazione (uguale a v1.1.1)
# ============================================================================
...
if connection_type == "Local (Ollama)":
    ...
    hosts_text = st.sidebar.text_area("Host (uno per riga)", value="http://192.168.1.10:11434/v1", height=80)
    ...
    provider = st.sidebar.selectbox("Provider", ["OpenAI", "Anthropic (Claude)", "Google Gemini", "Custom Cloud"])
    if provider == "OpenAI":
...
system_prompt = st.sidebar.text_area("System Prompt", value="Sei un assistente utile. Rispondi in modo chiaro.", height=80)
...
auto_save = st.sidebar.checkbox("Salvataggio automatico", value=st.session_state.get("auto_save_enabled", True), help="Salva automaticamente ad ogni messaggio")
...
if saved_conversations:
    ...
    selected_conv = st.sidebar.selectbox("Seleziona conversazione", options=[None] + [c[0] for c in conv_options], format_func=lambda x: "-- Seleziona --" if x is None else x)
...
# ============================================================================
# SIDEBAR - EXPORT CONVERSAZIONI (NUOVO v1.2.0)
# ============================================================================
...
st.info("✨ **Novità v1.2.0**: Export conversazioni in Markdown, JSON, TXT e PDF!")
...
user_msg = len([m for m in messages if m["role"] == "user"])
tokens = estimate_tokens_in_conversation()
...
if st.session_state.get("show_export_preview"):
    ...
    # Recupera opzioni export dalla sidebar (usa valori di default se non presenti)
...
with st.form("msg_form", clear_on_submit=True):
    user_input = st.text_area("Messaggio", "", height=100, placeholder="Scrivi... (Ctrl+Enter per inviare)", label_visibility="collapsed")
...
if submit and user_input.strip():
    if not model:
        ...
        try:
            ...
            history = get_conversation_history(max_messages if not include_full_history else None)
...
c2.caption("v1.2.0 - Export Conversations | Gilles DeepAiUG © 2025")
