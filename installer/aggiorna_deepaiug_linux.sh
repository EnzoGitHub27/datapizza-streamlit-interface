#!/bin/bash
# ============================================================
#  aggiorna_deepaiug_linux.sh
#  Script di aggiornamento DeepAiUG per Linux
...
echo "" > "$LOG"
echo "============================================================" >> "$LOG"
echo " DeepAiUG Updater Linux - Log di aggiornamento" >> "$LOG"
echo " Data: $(date)" >> "$LOG"
echo "============================================================" >> "$LOG"

echo ""
echo -e "${BOLD}============================================================${NC}"
echo -e "${BOLD}     DeepAiUG - Aggiornamento automatico Linux${NC}"
echo -e "${BOLD}============================================================${NC}"
echo ""
...
if [[ ! -d "$DEST" ]]; then
    check_err "DeepAiUG non trovato in: $DEST" \
        "Usa installa_deepaiug_linux.sh per la prima installazione."
    exit 1
fi

if [[ ! -f "$DEST/$VENV/bin/python" ]]; then
    check_err "Ambiente virtuale non trovato in: $DEST/$VENV" \
        "Usa installa_deepaiug_linux.sh per reinstallare."
    exit 1
fi
